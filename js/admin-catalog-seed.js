...
async function seedMissingApps(){
  ...
  await Promise.all(missing.map(a=>setDoc(doc(db,'apps',a.id),{name:a.name,category:a.category,version:'',description:a.description,iconUrl:'',platforms:a.platforms,telegramUrls:{},active:true,updatedAt:serverTimestamp()})));
  return missing.length;
}
function addSeedButton(){
  const head=document.querySelector('#apps .section-head');
  if(!head||document.getElementById('seedCatalogBtn'))return;
  const b=document.createElement('button');b.id='seedCatalogBtn';b.className='btn ghost';b.innerHTML='<i class="fa-solid fa-wand-magic-sparkles"></i> Initialize starter catalog';head.querySelector('#newAppBtn')?.before(b);
  b.onclick=async()=>{b.disabled=true;try{const n=await seedMissingApps();alert(n?`${n} starter applications are now real editable records.`:'The starter catalog is already editable.');location.reload()}catch(e){alert(e?.message||'Unable to initialize catalog.');b.disabled=false}};
}
document.addEventListener('DOMContentLoaded',addSeedButton);
onAuthStateChanged(auth,async user=>{if(!user)return;try{const s=await getDocs(collection(db,'apps'));if(!s.size)addSeedButton()}catch(e){console.warn('Catalog check failed',e)}});