(() => {
  ...
  const normalizePublicFlow = () => {
    ...
    document.querySelectorAll('a[href*="open=register"],a[href*="open=login"]').forEach(a => {
      a.href = 'payment.html';
      a.textContent = 'Subscribe & activate';
    });
    document.querySelectorAll('a,button').forEach(el => {
      const text = (el.textContent || '').trim();
      if (/^(Sign in|Sign out|Create account|Register|Forgot password)$/i.test(text)) {
        el.textContent = 'Subscribe & activate';
        if (el.tagName === 'A') el.href = 'payment.html';
      }
    });
  };

  const addMobileNav = () => {
    if (!isPublic || document.querySelector('.vision-bottom-nav')) return;
    const path = location.pathname.split('/').pop() || 'index.html';
    const active = key =>
      (key === 'home' && (path === '' || path === 'index.html')) ||
      (key === 'library' && path === 'library.html') ||
      (key === 'apple' && path === 'ios-store.html') ||
      (key === 'plans' && (path === 'payment.html' || path === 'activation.html'));
    ...
    window.addEventListener('scroll', () => {
      if (!ticking) { requestAnimationFrame(syncNav); ticking = true; }
    }, { passive: true });
  };
  ...
  const init = () => {
    ...
    if (button && menu) {
      ...
      menu.querySelectorAll('[data-theme-choice]').forEach(choice => choice.addEventListener('click', () => {
